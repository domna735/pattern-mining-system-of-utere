...
def _read_tickers(path: Path) -> list[str]:
    ...
    # Heuristic: treat numeric tickers as HK stock codes if the filename or
    # any ticker in the file suggests HK.
    name = path.name.lower()
    hk_hint = ("hk" in name) or any(t.upper().endswith(".HK") for t in raw)

    if hk_hint:
        return [_normalize_hk(t) for t in raw]
    return raw
    ...
    with completed_path.open("a", newline="", encoding="utf-8") as f_completed, incomplete_path.open(
        ...
        for ticker, df in iter_many_ohlc_yfinance(tickers, cfg=fetch_cfg):
            ...
            for w in windows:
                ...
                c, inc, uo = scan_dataframe_for_patterns(df, ticker=ticker, cfg=cfg_w, scan_start_date=scan_start_date)
            ...
            completed = [r for k, r in completed_by_key.items() if len(completed_support.get(k, set())) >= min_support]
            incomplete = [r for k, r in incomplete_by_key.items() if len(incomplete_support.get(k, set())) >= min_support]
